<x-app-layout :title="'Data Setoran Program Motor'">
    <div class="container mt-4">
        <h3 class="text-center">Data Setoran Program Motor <b>{{ $programMotor->member->id }}</b></h3>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($programMotor->setors as $program)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $program->program_motor_id }}</td>
        ...
        <x-button route="{{ route('program_motor.lunas', $programMotor->member_id) }}" color="primary mt-4" buttonText="Back" />
